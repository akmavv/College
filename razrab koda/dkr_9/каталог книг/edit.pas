  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[3, SG.RowCount-1]:= FloatToStr(MyCont.ISBN);
        SG.Cells[4, SG.RowCount-1]:= MyCont.Izdat;
        SG.Cells[5, SG.RowCount-1]:= IntToStr(MyCont.Stran);
    end;
  finally
    CloseFile(f);
  end;
...
begin
  ...
  try
    ...
    for i:= 1 to SG.RowCount-1 do begin
      ...
      MyCont.ISBN:= StrToFloat(SG.Cells[3, i]);
      MyCont.Izdat:= SG.Cells[4, i];
      MyCont.Stran:= StrToInt(SG.Cells[5, i]);
      //записываем их:
      Write(f, MyCont);
    end;
  finally
